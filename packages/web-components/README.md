...
| `floating-presence-stack` | Declarative timed stack rendering, native Top Layer, pause/resume, exit presence, and removal |
...
| `floating-query`, `floating-results` | Query input and rendered search phases/results |
| `floating-results-status`, `floating-results-item`, `floating-results-more` | Composable result phase, item, and pagination parts |
| `floating-composite`, `floating-composite-item` | Composite/grid roving focus; `item-selector` can register existing controls |
| `floating-delay-group`, `next-floating-delay-group` | Shared hover/focus delays |

`<floating-results>` is the preferred phase renderer. The previous
`<floating-search>` name remains registered as a deprecated compatibility
alias; migrate markup when convenient. The `floating-results-*` parts replace
the repeated native `template[data-search-*]` wrapper pattern. Keep
`data-search-text` and `data-search-load-more` inside a part when binding a
result field or load-more button, because those are declarative data bindings.
...
- [Getting started](https://fup.polcaneli.com/docs/getting-started)
