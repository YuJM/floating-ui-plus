...
<FloatingRoot v-model:open="open" :plugins="[click(), dismiss(), role({role: 'dialog'})]">
  ...
  <FloatingContent top-layer="popover">Settings <FloatingClose>Close</FloatingClose></FloatingContent>
</FloatingRoot>
...
input.replaceWith(Object.assign(input, query.getInputProps()));
query.bindOption(option, search.items[0]!, 0);
...
<floating-root placement="bottom-start">
  <floating-list navigation loop allow-escape>
    <floating-query data-destination-query>
      ...
      <p data-query-status aria-live="polite"></p>
    </floating-query>
  </floating-list>
</floating-root>
```
...
`useQuery()` returns `inputProps`, `getOptionProps()`, `statusText`, and
