              ...
              <section ref="floatingElement" class="modal-panel vue-modal" aria-labelledby="vue-modal-heading" aria-modal="true" v-bind="floating.floatingAttrs">
                ...
                <div class="modal-nested-actions">
                  <FloatingNode :controller="tooltip" id="vue-focus-room-tooltip">
                    <button ref="tooltipReference" class="vue-text-button" v-bind="tooltip.referenceAttrs">Show placement hint</button>
                    <FloatingPortal v-if="tooltipOpen" :active="tooltipOpen" :context-scope="tooltip.contextScope">
                      <div ref="tooltipElement" class="vue-tooltip" v-floating="tooltip" v-bind="tooltip.floatingAttrs">This tooltip stays inside the dialog.</div>
                    </FloatingPortal>
                  </FloatingNode>
                  <FloatingNode :controller="popover" id="vue-focus-room-popover">
                    <button ref="popoverReference" class="vue-text-button" v-bind="popover.referenceAttrs">Open room details</button>
                    <FloatingPortal v-if="popoverOpen" :active="popoverOpen" :context-scope="popover.contextScope">
                      <section ref="popoverElement" class="vue-popover" aria-label="Room details" v-floating="popover" v-bind="popover.floatingAttrs">
                        <span class="vue-panel-label">NESTED PORTAL</span>
                        <strong>Details stay above the dialog.</strong>
                        <p>Escape and outside press dismiss only this panel first.</p>
                        <button class="vue-text-button" @click="popoverOpen = false">Close details</button>
                      </section>
                    </FloatingPortal>
                  </FloatingNode>
                  <FloatingNode :controller="nested" id="vue-focus-room-child-dialog">
                    <button ref="nestedReference" class="vue-text-button" v-bind="nested.referenceAttrs">Open nested dialog</button>
